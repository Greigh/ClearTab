...
data class PeriodConfig(val mode: String, val startDay: Int, val length: Int) {
    companion object {
        fun normalized(mode: String?, startDay: Int?, length: Int?) = PeriodConfig(
...
object Period {
    ...
    fun bounds(date: LocalDate, cfg: PeriodConfig): PeriodBounds = when (cfg.mode) {
        ...
        "rolling" -> {
            ...
            val daysSince = ChronoUnit.DAYS.between(ROLL_EPOCH, date)
            val idx = Math.floorDiv(daysSince, len)
            val start = ROLL_EPOCH.plusDays(idx * len)
