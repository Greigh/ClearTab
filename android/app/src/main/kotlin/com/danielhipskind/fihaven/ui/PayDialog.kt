...
/**
 * Record a payment toward a bill/card with quick presets — Full for bills,
 * Minimum / Recommended for cards — plus a custom amount and a goal hint.
 * Payments accumulate toward the monthly goal. Mirrors the web pay modal.
 */
@Composable
fun PayDialog(vm: AppViewModel, type: String, refId: String, name: String, onDismiss: () -> Unit) {
    ...
    val presets: List<PayPreset> = remember(data, type, refId) {
        if (type == "bill") {
            ...
            if (card == null) emptyList()
                ...
                if (rec > card.minPayment + Schedule.PAID_EPSILON) {
                    val sub = when {
                        (card.recommendedPayment ?: 0.0) > 0.0 -> "Your set payment"
    ...
    val initial = (goal - alreadyPaid).coerceAtLeast(0.0).let {
    ...
    var dateStr by remember { mutableStateOf("%04d-%02d-%02d".format(today.year, today.monthValue, today.dayOfMonth)) }
    var note by remember { mutableStateOf("") }

    val amountVal = amount.toDoubleOrNull() ?: 0.0

    val policyLabel = if (type == "bill") "full amount" else when (vm.paidGoalPolicy()) {
    ...
    val hint = when {
        ...
        projected >= goal - Schedule.PAID_EPSILON ->
            ...
            val soFar = if (alreadyPaid > Schedule.PAID_EPSILON) " Already paid ${Money.fmt(alreadyPaid)} this month." else ""
    ...
    FormDialog(
        ...
        OutlinedTextField(
            amount, { amount = it }, label = { Text("Amount paid") }, prefix = { Text("$") },
            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Decimal),
            singleLine = true, modifier = Modifier.fillMaxWidth(),
        )
        if (hint.isNotEmpty()) {
            Text(hint, color = if (projected >= goal - Schedule.PAID_EPSILON) Ct.colors.green else Ct.colors.muted, fontSize = 12.sp)
        }
        OutlinedTextField(
            dateStr, { dateStr = it }, label = { Text("Date paid (YYYY-MM-DD)") },
            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
            singleLine = true, modifier = Modifier.fillMaxWidth(),
        )
        OutlinedTextField(
            note, { note = it }, label = { Text("Note (optional)") },
            modifier = Modifier.fillMaxWidth(),
        )
    }
}
...
private fun PresetLabel(p: PayPreset, onAccent: Boolean) {
    ...
    Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        Text(Money.fmt(p.amount), color = main, fontFamily = PlexMono, fontWeight = FontWeight.Medium)
    }
}
